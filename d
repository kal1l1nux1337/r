...
if not isfolder("oreo.c0m\\Themes") then
	pcall(makefolder, "oreo.c0m")
	pcall(makefolder, "oreo.c0m\\Themes")
end
...
for i, v in next, themes do
	if i ~= "selected" then
		writefile("oreo.c0m\\Themes\\" .. i .. ".json", jsonEncodeTheme(v))
	end
end
...
function library.new(gameName)
	local lib = setmetatable({
		_name = gameName,
		_gui = create("ScreenGui", { Name = "oreo.c0m", ZIndexBehavior = Enum.ZIndexBehavior.Sibling }, {
			create("Frame", { Name = "main", BackgroundColor3 = "theme.mainBackground", ClipsDescendants = true, Position = UDim2.new(0, 100, 0, 100), Size = UDim2.new(0, 535, 0, 380) }, {
				...
				create("Frame", { Name = "top", BackgroundColor3 = "theme.topBackground", Size = UDim2.new(1, 0, 0, 34), ZIndex = 2 }, {
					create("TextLabel", { Name = "title", BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0, 34, 0, 0), Size = UDim2.new(1, -68, 1, 0), Text = "oreo.c0m | " .. gameName, TextColor3 = "theme.textForeground", TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left }),
...
function library:GetConfigs()
	local configs = {}
	if isfolder("oreo.c0m\\Configs\\" .. self._name) then
		for i, v in next, listfiles("oreo.c0m\\Configs\\" .. self._name) do
...
function library:LoadConfig(name)
	local filePath = "oreo.c0m\\Configs\\" .. self._name .. "\\" .. name .. ".json"
	if not (isfolder("oreo.c0m\\Configs\\" .. self._name) and isfile(filePath)) then
		return
	end
...
function library:SaveConfig(name)
	if not isfolder("oreo.c0m\\Configs\\" .. self._name) then
		pcall(makefolder, "oreo.c0m")
		pcall(makefolder, "oreo.c0m\\Configs")
		pcall(makefolder, "oreo.c0m\\Configs\\" .. self._name)
	end
	local config = {}
	for _, tab in next, self._tabs do
		for _, panel in next, tab._panels do
	...
	writefile("oreo.c0m\\Configs\\" .. self._name .. "\\" .. name .. ".json", httpService:JSONEncode(config))
end
...
function library:GetThemes()
	...
	for i, v in next, listfiles("oreo.c0m\\Themes") do
		local split = select(-1, unpack(v:split("\\")))
		if split:find(".json") then
...
function library:LoadTheme(name)
	local filePath = "oreo.c0m\\Themes\\" .. name .. ".json"
	if not (isfolder("oreo.c0m\\Themes") and isfile(filePath)) then
		print("noes")
		return
	end
...
function library:Notify(text, callback, options)
	...
	local frame = create("Frame", { Name = "notification", AnchorPoint = Vector2.new(1, 1), BackgroundColor3 = "theme.panelItemBackground", ClipsDescendants = true, Parent = self._gui.notifs, Position = UDim2.new(1, 300, 1, -30), Size = UDim2.new(0, 280, 0, sizeY + 34) }, {
		create("TextLabel", { Name = "title", BackgroundTransparency = 1, Font = Enum.Font.GothamSemibold, Position = UDim2.new(0, 10, 0, 0), Size = UDim2.new(1, -66, 0, 30), Text = "oreo.c0m Notification", TextColor3 = "theme.textForeground", TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left }),
